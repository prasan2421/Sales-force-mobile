...
class New extends Component {
    ...
    getCustomers = async () => {
        ...
        fetch('http://14.192.18.73/api/customers', {
            ...
            .catch((error) => {
                alert('Get customer error occurred');
            })
    ...
    handleSearchValue(text){
        if(text==''){
            ...
            let dataList= this.state.datafull.filter(cat=>cat.name.toLowerCase().indexOf(text.toLowerCase()) > -1);
                    ...
                    return (
                        ...
                        >
                            <TouchableHighlight
                                ...
                                flexDirection: 'row',
                                width: '95%',
                                backgroundColor: this.state.checkinId == item._id ?'#ff650a':'#ffffff',
                                elevation: 1,
                                padding: 10,
                            ...
                            >
                                <View style={{}}>
                                    <Text style={{fontSize: 14,color:this.state.checkinId == item._id ?'#fff':'#2a8ee5'}}>{item.name}</Text>
                                    <Text style={{fontSize: 12,color:this.state.checkinId == item._id ?'#fff':'#1f1f1f'}}>{item.billing_address}</Text>
                                    <Text style={{fontSize: 14,color:this.state.checkinId == item._id ?'#fff':'#1f1f1f'}}>{item.owner_contact_number}</Text>
                                    <Text style={{fontSize: 14,color:this.state.checkinId == item._id ?'#fff':'#e6407f'}}>{item.customer_type}</Text>
                                </View>
        ...
        return (
            <ScrollView style={{backgroundColor:'#ebeef3'}}>
                ...
                <View style={{width:'100%',marginBottom:10,alignItems: 'center',marginTop:10,}}>
                    ...
                    <View style={{flexDirection:'row',width:'95%',backgroundColor:'#fff'}}>

                        <TextInput style={{paddingLeft:10,width:'100%'}}
                                   placeholder="Search Shop ..."
                                   returnKeyType='done'
                                   onSubmitEditing={Keyboard.dismiss}
                                   onChangeText={(text) => this.handleSearchValue(text)}
                ...
                <View style={{width:'100%',marginBottom:10,alignItems: 'center'}}>
                    <View style={{width:'95%',flexDirection:'row',justifyContent:'space-between'}}>
                        <Text style={{color:'#f9156c'}}>All ></Text>
                        <Text style={{color:'#f9156c'}}>Total shop in {this.state.beat}:{this.state.count}</Text>
                    </View>
                </View>
