...
class New extends Component {
    ...
    getApprovalFromAsyncStorage = async () => {
        ...
        fetch('http://imsnepal.com:1660/api/dispatch/getdispatchdetails?refno='+this.state.REFNO, {
            headers: {
...
// console.warn(data);
        ...
        const access_token = await AsyncStorage.getItem('access_token');
        let url = 'http://imsnepal.com:1660/api/Dispatch/postSalesApprove';
        this.showLoader();
        fetch(url, {
            method: 'POST', // or 'PUT'
            ...
            .then((responseJson) => {
                if(responseJson.status!='ok') {
                    ...
                    this.hideLoader();
                    this.props.navigation.navigate('SalesDispatch', { action: 'refresh' })
                }
            })
            .catch((error) => {
                console.error(error);
...
// console.warn(data);
        ...
        const access_token = await AsyncStorage.getItem('access_token');
        let url = 'http://imsnepal.com:1660/api/Dispatch/declinePO';
        this.showLoader();
        fetch(url, {
            method: 'POST', // or 'PUT'
            body: data, // data can be `string` or {object}!
...
// console.warn(data);
        ...
        const access_token = await AsyncStorage.getItem('access_token');
        let url = 'http://imsnepal.com:1660/api/dispatch/saveDispatch';
        this.showLoader();
        fetch(url, {
            method: 'POST', // or 'PUT'
            body: data, // data can be `string` or {object}!
...
// console.warn(data);
        ...
        const access_token = await AsyncStorage.getItem('access_token');
        let url = 'http://imsnepal.com:1660/api/dispatch/updateDispatch';
        this.showLoader();
        fetch(url, {
            method: 'POST', // or 'PUT'
            body: data, // data can be `string` or {object}!
                ...
                <View style={{alignItems:'center',marginBottom:15,marginTop:15}}>
                    <View style={{marginLeft:20,width:'100%'}}>
                        ...
                        <View style={{flexDirection:'row',}}>
                            <Text style={{fontSize:12,width:'35%'}}>Order Qty:{item.orderQty} </Text>
                        </View>

                        <View style={{flexDirection:'row',alignItems:'center'}}>
                            <Text style={{fontSize:12,width:'35%'}}>Dispatch Qty: </Text>
                            <TextInput
